...
async def ingest(collection_id):
    ...
    document_id = None
    async for ingest_info in insert_document(None, collection_id, doc, unique_filename):
...
async def logged_lifecycle(shabti_client, log_file):
    ...
    document_id = await ingest(collection.collection_id)
    await delete_document(None, collection.collection_id, document_id)
...
async def test_an_unsecured_instance_logs_no_user(logged_lifecycle):
    _, entries = logged_lifecycle
    # there is nobody to attribute an action to without Keycloak, so the key is simply absent
    assert not any("user" in entry for entry in entries)


async def test_the_logged_collection_is_a_plain_collection_info(logged_lifecycle):
    collection_id, entries = logged_lifecycle
    for entry in entries:
        assert set(entry["collection"]) == {"collection_name", "collection_id"}
